...
int call_rsa_gen_via_old(size_t keybits, unsigned long exp,
    ...
    // 3. generate key
    if (!RSA_generate_key_ex(rsa, keybits, bn_exp, NULL)) {
    ...
    if (!i2d_RSA_PUBKEY_bio(bio_pub, rsa)) {
        fprintf(stderr, "i2d_RSA_PUBKEY_bio failed: %s\n", ERR_error_string(ERR_get_error(), NULL));
        ret = -1;
        goto out;
    }
    ...
    *publen = bio_len_pub;
    ...
    if (!i2d_RSAPrivateKey_bio(bio_pri, rsa)) {
        fprintf(stderr, "i2d_RSAPrivateKey_bio failed: %s\n", ERR_error_string(ERR_get_error(), NULL));
        ret = -1;
        goto out;
    }
...
int call_rsa_gen_via_evp(size_t keybits, unsigned long exp,
    ...
    // 4. set public exponent
    // Note: OpenSSL uses BIGNUM for exponent, but EVP_PKEY_CTX_set_rsa_keygen_pubexp expects a long
    // default is RSA_F4 (65537)
    // if set exp, free by EVP_PKEY_CTX_free, do not free manually
    bn_exp = BN_new();
    if (!bn_exp) {
        fprintf(stderr, "BN_new failed: %s\n", ERR_error_string(ERR_get_error(), NULL));
        ret = -1;
    ...
    if (EVP_PKEY_CTX_set_rsa_keygen_pubexp(ctx, bn_exp) <= 0) {
        fprintf(stderr, "EVP_PKEY_CTX_set_rsa_keygen_pubexp failed: %s\n", ERR_error_string(ERR_get_error(), NULL));
        ret = -1;
        goto out;
    }
...
int main(int argc, char *argv[]) {
    ...
    ret = call_rsa_gen_via_old(keybits, exp, &pub, &publen, &pri, &prilen);
    if (ret != 0) {
        fprintf(stderr, "call_rsa_gen_via_old failed\n");
        return ret;
    }
    // print public key
    ...
    ret = call_rsa_gen_via_evp(keybits, exp, &pub, &publen, &pri, &prilen);
    if (ret != 0) {
        fprintf(stderr, "call_rsa_gen_via_evp failed\n");
        return ret;
    }
