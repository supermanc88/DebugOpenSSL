...
int call_digest_sha2(const char *md_string, const char *msg, unsigned char *md_value, unsigned int *md_len) {
    int ret = 0;
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; md_vectors[i].md_string != NULL; i++) {
        ...
        ret = call_digest_sha2(md_vec->md_string, msg, md_value, &md_len);
        if (ret != 0) {
            fprintf(stderr, "call_digest_sha2 failed for %s\n", md_vec->md_string);
            goto out;
        }
