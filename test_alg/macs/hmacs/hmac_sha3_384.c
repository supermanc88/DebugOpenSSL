...
int call_hmac_sha3_384(unsigned char *key, int keylen,
    ...
    // Get the sha3_384 digest
    md = EVP_sha3_384();
    if (!md) {
        fprintf(stderr, "Failed to get SHA3_384 digest: %s\n", ERR_error_string(ERR_get_error(), NULL));
        ret = -1;
        goto out;
    }
...
int main(int argc, char *argv[]) {
    ...
    ret = call_hmac_sha3_384(key, strlen((char *)key), data, strlen((char *)data), out_data, &out_datalen);
    if (ret != 0) {
        fprintf(stderr, "HMAC-SHA3_384 computation failed\n");
        return ret;
    }
    printf("HMAC-SHA3_384: ");
