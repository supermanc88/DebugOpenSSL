...
int call_sm4_ofb_encrypt(unsigned char *key,
                         unsigned char *iv,
    ...
    // Get the SM4 OFB cipher, we can use EVP_sm4_ofb() directly
    cipher = EVP_sm4_ofb();
    if (!cipher) {
        fprintf(stderr, "Failed to get SM4-OFB cipher: %s\n", ERR_error_string(ERR_get_error(), NULL));
        ret = -1;
        goto out;
    }
...
int call_sm4_ofb_decrypt(unsigned char *key,
                         unsigned char *iv,
    ...
    // get the cipher, for OFB mode, we can use EVP_sm4_ofb() directly
    cipher = EVP_sm4_ofb();
    if (!cipher) {
        fprintf(stderr, "Failed to get SM4-OFB cipher: %s\n", ERR_error_string(ERR_get_error(), NULL));
        ret = -1;
...
int main(int argc, char *arv[]) {
    ...
    unsigned char plaintext[] = "Hello, this is a test message for SM4 OFB mode encryption!";
    unsigned char ciphertext[128] = {0};
    unsigned char decryptedtext[128] = {0};
    size_t decryptedtext_len = 0;
    size_t ciphertext_len = 0;

    ret = call_sm4_ofb_encrypt(key, iv, plaintext, strlen((char *)plaintext), ciphertext, &ciphertext_len);
    if (ret != 0) {
        fprintf(stderr, "SM4 OFB encryption failed\n");
        return ret;
    }
    ...
    ret = call_sm4_ofb_decrypt(key, iv, ciphertext, ciphertext_len, decryptedtext, &decryptedtext_len);
    if (ret != 0) {
        fprintf(stderr, "SM4 OFB decryption failed\n");
        return ret;
    }
